...
from .cifar import CIFAR10 #Omniglot  # patched to use memoization
...
class CifarSampler:
    def __init__(self, root, preload_train=False, preload_test=False, im_size=28):
        ...
        self.chars_train = np.arange(len(self.cifar_train._flat_character_images))
        ...
        self.chars_test = np.arange(len(self.cifar_test._flat_character_images))
    ...
    def sample_train(self, remember_size=64, device="cuda"):
        ...
        samples = [self.cifar_train[i] for i in range(task_id * 20, (task_id + 1) * 20)]
        ...
        samples = [self.cifar_train[i] for i in ids]
    ...
    def sample_test(self, num_tasks, train_examples=15, device="cuda"):
        ...
        train_traj = [
            self.cifar_test[i] for train_task in train_tasks for i in train_task
        ]

        test_traj = [self.cifar_test[i] for test_task in test_tasks for i in test_task]
